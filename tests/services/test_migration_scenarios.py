...
from src.models.migration_target import PlatformType, MigrationStrategy
...
class TestMigrationScenarioService:
    ...
    def test_create_target(self, service):
        ...
        target = service.create_target(name="Test Target", platform_type=PlatformType.GCP, region="us-central1")
    ...
    def test_calculate_migration_cost(self, service, sample_vms, aws_target):
        ...
        cost_breakdown = service.calculate_migration_cost(vms=sample_vms, target=aws_target, duration_days=30)
    ...
    def test_calculate_migration_duration(self, service, sample_vms, aws_target):
        ...
        duration = service.calculate_migration_duration(vms=sample_vms, target=aws_target, parallel_migrations=2)
    ...
    def test_assess_risk_level_low(self, service, aws_target):
        ...
        small_vms = [
            VirtualMachine(vm=f"vm-{i}", cpus=2, memory=4096, provisioned_mib=10240) for i in range(5)  # 10 GB
        ]

        risk_level, risk_factors = service.assess_risk_level(
            vms=small_vms, target=aws_target, strategy=MigrationStrategy.REHOST
        )
    ...
    def test_assess_risk_level_high(self, service, aws_target):
        ...
        large_vms = [
            VirtualMachine(vm=f"vm-{i}", cpus=8, memory=16384, provisioned_mib=1048576) for i in range(150)  # 1 TB
        ]

        risk_level, risk_factors = service.assess_risk_level(
            vms=large_vms, target=aws_target, strategy=MigrationStrategy.REFACTOR
        )
    ...
    def test_calculate_recommendation_score(self, service, aws_target):
        ...
        cost_breakdown = {"compute": 1000, "memory": 500, "storage": 200, "network": 100, "labor": 1500, "total": 3300}
        duration = {"total_days": 5}

        score = service.calculate_recommendation_score(
            cost_breakdown=cost_breakdown, duration=duration, risk_level="LOW", target=aws_target
        )
    ...
    def test_generate_migration_waves(self, service, sample_vms, aws_target):
        ...
        waves = service.generate_migration_waves(scenario_id=scenario.id, wave_size=2, strategy="size_based")
