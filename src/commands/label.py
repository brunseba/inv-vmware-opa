...
@click.option("--db-url", default="sqlite:///data/vmware_inventory.db", help="Database URL", show_default=True)
...
def list_labels(key: str, db_url: str):
    ...
    try:
        ...
        table_data = []
        for lbl in labels:
            table_data.append([lbl.id, lbl.key, lbl.value, lbl.description or "", lbl.color or ""])
...
def assign_vm_label(vm_name: str, key: str, value: str, by: str, db_url: str):
    ...
    try:
        ...
        # Find VM
        vm = service.session.query(VirtualMachine).filter(VirtualMachine.vm == vm_name).first()
...
@click.argument("vm_name")
@click.option("--inherited/--no-inherited", default=True, help="Include inherited labels")
@click.option("--db-url", default="sqlite:///data/vmware_inventory.db", help="Database URL", show_default=True)
def list_vm_labels(vm_name: str, inherited: bool, db_url: str):
    ...
    try:
        ...
        for lbl in labels:
            ...
            table_data.append([lbl["key"], lbl["value"], source, lbl["assigned_by"] or ""])
...
@label.command("clean-all")
@click.option(
    "--include-inherited/--no-include-inherited",
    default=True,
    help="Include inherited labels from folders",
    show_default=True,
)
@click.option("--db-url", default="sqlite:///data/vmware_inventory.db", help="Database URL", show_default=True)
@click.confirmation_option(prompt="⚠️  This will remove ALL VM label assignments. Are you sure?")
def clean_all_vm_labels(include_inherited: bool, db_url: str):
    """Remove all label assignments from VMs.

    This command removes all VM label assignments from the database.
    Label definitions themselves are preserved and can be reassigned later.

    Use --no-include-inherited to only remove direct label assignments
    and keep inherited labels from folders.
    """
    try:
        service = get_label_service(db_url)
        ...
        total_before = service.session.query(VMLabel).count()

        if total_before == 0:
            click.echo("ℹ️  No VM label assignments found to remove.")
            return

        inherited_count = service.session.query(VMLabel).filter(VMLabel.inherited_from_folder.is_(True)).count()
        ...
        if not include_inherited:
            click.echo("ℹ️  Only direct assignments will be removed (inherited labels preserved).\n")
        ...
        click.echo("💡 Label definitions have been preserved and can be reassigned.")
...
@click.argument("key")
@click.argument("value")
@click.option("--inherit-vms/--no-inherit-vms", default=True, help="Apply label to VMs in folder")
@click.option("--inherit-subfolders/--no-inherit-subfolders", default=False, help="Apply to subfolders recursively")
@click.option("--by", help="Assigned by (user name)")
@click.option("--db-url", default="sqlite:///data/vmware_inventory.db", help="Database URL", show_default=True)
def assign_folder_label(
    folder_path: str, key: str, value: str, inherit_vms: bool, inherit_subfolders: bool, by: str, db_url: str
):
    ...
    try:
        ...
        service.assign_folder_label(
            folder_path, label.id, assigned_by=by, inherit_to_vms=inherit_vms, inherit_to_subfolders=inherit_subfolders
        )
...
@click.argument("key")
@click.argument("value")
@click.option("--keep-inherited/--remove-inherited", default=False, help="Keep inherited labels on VMs")
@click.option("--db-url", default="sqlite:///data/vmware_inventory.db", help="Database URL", show_default=True)
def remove_folder_label(folder_path: str, key: str, value: str, keep_inherited: bool, db_url: str):
    ...
    try:
        ...
        # Remove from folder
        success = service.remove_folder_label(folder_path, label.id, remove_inherited=not keep_inherited)
...
def list_folder_labels(folder_path: str, db_url: str):
    ...
    try:
        ...
        for lbl in labels:
            ...
            table_data.append([lbl["key"], lbl["value"], inherit, lbl["assigned_by"] or ""])
...
@click.option("--clear-existing", is_flag=True, help="Clear all existing labels before restore (DANGEROUS!)")
@click.option("--db-url", default="sqlite:///data/vmware_inventory.db", help="Database URL", show_default=True)
...
@click.option("--inherit-vms/--no-inherit-vms", default=True, help="Apply label to VMs in folders")
@click.option("--inherit-subfolders/--no-inherit-subfolders", default=False, help="Apply to subfolders recursively")
@click.option("--by", help="Assigned by (user name)")
@click.option("--dry-run", is_flag=True, help="Show what would be done without applying")
@click.option("--db-url", default="sqlite:///data/vmware_inventory.db", help="Database URL", show_default=True)
def assign_all_folders_label(
    key: str, value: str, pattern: str, inherit_vms: bool, inherit_subfolders: bool, by: str, dry_run: bool, db_url: str
):
