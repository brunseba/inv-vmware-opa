...
@click.confirmation_option(prompt="This will create an anonymized copy of the database. Continue?")
def database(
    db_url: str,
    output: Path,
    ...
    try:
        ...
        with Progress(
            ...
            while offset < total_vms:
                ...
                for vm in vms:
                    # Convert to dict
                    vm_dict = {c.name: getattr(vm, c.name) for c in vm.__table__.columns if c.name != "id"}
...
def excel(
    ...
    if generate_mapping_template:
        try:
            ...
            ColumnMapper.generate_template(
                excel_file=excel_file, output_file=generate_mapping_template, sheet=sheet, include_samples=True
            )
    ...
    if list_columns:
        try:
            ...
            console.print()
            console.print(
                f"  Example: cli anonymize excel {excel_file.name} -o output.xlsx -f 'Column Name' -f 'Another Column'"
            )
            console.print()
            return
        except Exception as e:
    ...
    try:
        ...
        # Map columns
        df, mapping_result = column_mapper.map_columns(df, expected_fields=set(ANONYMIZABLE_FIELDS.keys()))
