...
def show_convention(convention_id: int, db_url: str):
    ...
    try:
        ...
        if convention.fields:
            ...
            headers = ["Pos", "Field Name", "Length", "Required", "Values"]
...
def analyze_inventory(convention_id: int, db_url: str, datacenter: Optional[str], cluster: Optional[str], batch_size: int):
    ...
    try:
        ...
        click.echo(f"   Valid:         {stats['valid']} ({stats['valid']/stats['total']*100:.1f}%)" if stats['total'] > 0 else "   Valid:         0")
        click.echo(f"   Invalid:       {stats['invalid']} ({stats['invalid']/stats['total']*100:.1f}%)" if stats['total'] > 0 else "   Invalid:       0")
...
def migration_groups(convention_id: int, db_url: str, group_by: tuple, format: str):
    ...
    try:
        ...
        groups = service.generate_migration_groups(
            convention_id=convention_id,
