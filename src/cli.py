...
def get_default_db_url() -> str:
    ...
    Checks VMWARE_INV_DB_URL environment variable first, otherwise
    returns relative path default.
    """
    return os.environ.get("VMWARE_INV_DB_URL", "sqlite:///data/vmware_inventory.db")
...
def vm_list(db_url: str, datacenter: str, cluster: str, limit: int):
    ...
    try:
        ...
        for vm in vms:
            click.echo(
                "{:<40} {:<15} {:<20} {:<20}".format(
                    vm.vm[:40], vm.powerstate or "N/A", vm.datacenter or "N/A", vm.cluster or "N/A"
                )
            )
...
def clusters(db_url: str, datacenter: str, cluster_filter: str):
    ...
    try:
        ...
        results = (
            query.group_by(VirtualMachine.datacenter, VirtualMachine.cluster)
            .order_by(VirtualMachine.datacenter, func.count(VirtualMachine.id).desc())
            .all()
        )
...
def schema(db_url: str, table: str, filters: tuple, group_by: str):
    ...
    if table == "all":
        tables_to_show = [
            ("virtual_machines", vm_categories),
            ("labels", label_categories),
            ("vm_labels", vm_label_categories),
            ("folder_labels", folder_label_categories),
        ]
...
def _display_table_schema(inspector, table_name: str, categories: dict, filters: tuple, group_by: str):
    ...
    if group_by:
        ...
        for key, group in groupby(column_info, key=lambda x: x[group_by]):
            ...
            click.echo("{:<30} {:<20} {:<10} {:<10} {:<15}".format("Column", "Type", "Nullable", "Indexed", "Category"))
            click.echo("-" * 85)
        ...
        # Display flat list
        click.echo("\n{:<30} {:<20} {:<10} {:<10} {:<15}".format("Column", "Type", "Nullable", "Indexed", "Category"))
        click.echo("-" * 85)
...
@click.option(
    "--db-url",
    default=None,
    help="Database URL (default: from VMWARE_INV_DB_URL env or sqlite:///data/vmware_inventory.db)",
)
