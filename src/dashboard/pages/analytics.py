    ...
    try:
        ...
        if vms:
            ...
            with col2:
                ...
                df_vms["Config"] = (
                    df_vms["CPUs"].astype(str)
                    + " CPU / "
                    + df_vms["Memory_GB"].round(0).astype(int).astype(str)
                    + " GB"
                )
        ...
        vms_with_dates = session.query(VirtualMachine).filter(VirtualMachine.creation_date.isnot(None)).all()

        if vms_with_dates:
            df_dates = pd.DataFrame(
                [
                    {"Date": vm.creation_date.date(), "VM": vm.vm, "Datacenter": vm.datacenter or "Unknown"}
            ...
            fig = px.line(monthly_counts, x="Month", y="Count", title="VMs Created Over Time", markers=True)
            fig.update_layout(xaxis_title="Month", yaxis_title="VMs Created")
            fig = ThemeManager.apply_chart_theme(fig)
        ...
        if cluster_data:
            df_clusters = pd.DataFrame(cluster_data, columns=["Cluster", "VMs", "Hosts", "CPUs", "Memory_MB"])
            ...
            with col1:
                ...
                fig.update_layout(showlegend=False, xaxis_tickangle=-45, yaxis_title="VMs per Host")
                fig = ThemeManager.apply_chart_theme(fig)
            ...
            with col2:
                ...
                fig.update_layout(xaxis_title="Total vCPUs", yaxis_title="Total Memory (GB)")
                fig = ThemeManager.apply_chart_theme(fig)
        ...
        env_data = (
            session.query(VirtualMachine.env, func.count(VirtualMachine.id).label("count"))
...
def render_custom_explorer(db_url: str):
    ...
    col1, col2 = st.columns(2)
    with col1:
        limit = st.selectbox("Maximum Rows", options=[1000, 5000, 10000, 25000], index=2, key="analytics_limit")
    with col2:
        include_templates = st.checkbox("Include Templates", value=False, key="analytics_templates")
                    ...
                    [
                        {
                            ...
                            "Storage_Efficiency_%": (
                                round(((vm.in_use_mib or 0) / (vm.provisioned_mib or 1)) * 100, 1)
                                if vm.provisioned_mib and vm.provisioned_mib > 0
                                else 0
                            ),
                            "Datacenter": vm.datacenter or "Unknown",
                            "Cluster": vm.cluster or "Unknown",
