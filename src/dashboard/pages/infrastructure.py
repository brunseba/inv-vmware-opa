...
def render(db_url: str):
    ...
    try:
        ...
        if dc_stats:
            ...
            # Detailed table
            st.dataframe(
                df_dc.style.format({"VMs": "{:,.0f}", "CPUs": "{:,.0f}", "Memory_GB": "{:,.1f}"}),
                width="stretch",
                hide_index=True,
            )
        ...
        if cluster_stats:
            df_cluster = pd.DataFrame(cluster_stats, columns=["Cluster", "Datacenter", "VMs", "CPUs", "Memory_MB"])
        ...
        host_stats = (
            session.query(VirtualMachine.host, VirtualMachine.cluster, func.count(VirtualMachine.id).label("vm_count"))
            .group_by(VirtualMachine.host, VirtualMachine.cluster)
