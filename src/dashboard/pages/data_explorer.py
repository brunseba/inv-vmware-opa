...
def render(db_url: str):
    ...
    **Welcome to the Data Explorer!** This tool empowers you to create custom visualizations without writing code.
    ...
    colored_header(label="Load Configuration", description="Configure data loading options", color_name="violet-70")
    ...
    with col2:
        include_templates = st.checkbox("Include Templates", value=False, help="Include VM templates in the dataset")

    with col3:
        power_filter = st.selectbox(
            "Power State Filter",
            options=["All", "poweredOn", "poweredOf", "suspended"],
            index=0,
    ...
    with st.spinner(f"Loading up to {limit:,} VMs..."):
        try:
            ...
            try:
                ...
                pyg.walk(df, env="Streamlit", spec="./configs/data_explorer.json", use_kernel_calc=True, dark=theme)
