...
def render(db_url: str):
    ...
    try:
        ...
        with col1:
            analysis_mode = st.radio("Analysis Type", ["Summary", "Detailed"], horizontal=True)
        with col2:
...
def _render_summary_report(session, columns, total_vms, show_charts):
    ...
    for col_name in columns:
        try:
            ...
            non_null_count = session.query(func.count(col_attr)).filter(col_attr.isnot(None)).scalar() or 0

            # Count distinct values
            distinct_count = (
                session.query(func.count(func.distinct(col_attr))).filter(col_attr.isnot(None)).scalar() or 0
            )
    ...
    st.download_button(label="⬇️ Export Report CSV", data=csv_data, file_name="data_quality_report.csv", mime="text/csv")
    ...
    if show_charts:
        ...
        with col2:
            ...
            fig.update_layout(xaxis_type="log", xaxis_title="Unique Values (log scale)")
            fig = ThemeManager.apply_chart_theme(fig)
...
def _render_detailed_report(session, columns, total_vms):
    ...
    try:
        ...
        total_distinct = session.query(func.count(func.distinct(col_attr))).filter(col_attr.isnot(None)).scalar() or 0
        ...
        pagination = PaginationHelper(key_prefix=f"data_quality_detail_{selected_column}", default_page_size=25)

        # Build base query
        base_query = (
            session.query(col_attr, func.count(VirtualMachine.id).label("count"))
        ...
        # Display table
        st.dataframe(
            df_display.style.format({"Count": "{:,}", "Percentage": "{:.2f}%"}), width="stretch", hide_index=True
        )
        ...
        if viz_type == "Bar Chart":
            ...
            fig = px.pie(df_display, values="Count", names="Value", title=f"Top {limit} Values Distribution")
...
def _render_label_quality_report(session, total_vms):
    ...
    vms_with_labels = (
        session.query(func.count(func.distinct(VMLabel.vm_id))).filter(VMLabel.inherited_from_folder == False).scalar()
        or 0
    )

    # Get folder label assignments
    folders_with_labels = session.query(func.count(func.distinct(FolderLabel.folder_path))).scalar() or 0
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📊 Label Coverage", "🔑 Label Keys Analysis", "⚠️ Quality Issues", "📈 Coverage by Folder"]
    )
    ...
    with tab1:
        ...
        vms_no_labels = (
            session.query(func.count(VirtualMachine.id)).outerjoin(VMLabel).filter(VMLabel.id.is_(None)).scalar() or 0
        )

        # VMs with inherited labels only
        vms_inherited_only = (
            session.query(func.count(func.distinct(VMLabel.vm_id)))
            .filter(VMLabel.inherited_from_folder == True)
            .scalar()
            or 0
        )
        ...
        with col1:
            st.dataframe(coverage_data.style.format({"Count": "{:,}", "Percentage": "{:.1f}%"}), hide_index=True)
    ...
    with tab2:
        ...
        total_label_keys_count = session.query(func.count(func.distinct(Label.key))).scalar() or 0
        ...
        label_key_pagination = PaginationHelper(key_prefix="data_quality_label_keys", default_page_size=25)
        ...
        base_key_query = (
            ...
            .outerjoin(VMLabel, Label.id == VMLabel.label_id)
            .group_by(Label.key)
            .order_by(func.count(func.distinct(VMLabel.vm_id)).desc())
        )
        ...
        if key_stats:
            ...
            # Show table
            st.dataframe(
                df_keys.style.format({"Value Count": "{:,}", "VM Count": "{:,}", "VM Coverage %": "{:.1f}%"}),
                hide_index=True,
                width="stretch",
            )
    ...
    with tab3:
        ...
        single_value_keys = (
            session.query(Label.key).group_by(Label.key).having(func.count(func.distinct(Label.value)) == 1).all()
        )
        ...
        unused_labels = (
            session.query(func.count(Label.id))
            .outerjoin(VMLabel, Label.id == VMLabel.label_id)
            .outerjoin(FolderLabel, Label.id == FolderLabel.label_id)
            .filter(VMLabel.id.is_(None), FolderLabel.id.is_(None))
            .scalar()
            or 0
        )
        ...
        low_coverage_keys = (
            session.query(Label.key)
            .join(VMLabel, Label.id == VMLabel.label_id)
            .group_by(Label.key)
            .having(func.count(func.distinct(VMLabel.vm_id)) < total_vms * 0.1)  # Less than 10% coverage
            .all()
        )
    ...
    with tab4:
        ...
        # Count total folders
        total_folders = (
            session.query(func.count(func.distinct(VirtualMachine.folder)))
        ...
        folder_pagination = PaginationHelper(key_prefix="data_quality_folder_coverage", default_page_size=50)
        ...
        base_folder_query = (
            ...
            .outerjoin(VMLabel, VirtualMachine.id == VMLabel.vm_id)
            .filter(VirtualMachine.folder.isnot(None))
            .group_by(VirtualMachine.folder)
            .order_by(func.count(func.distinct(VirtualMachine.id)).desc())
        )
        ...
        if folder_coverage:
            df_folders = pd.DataFrame(folder_coverage, columns=["Folder", "Total VMs", "Labeled VMs"])
            ...
            df_filtered = df_folders[df_folders["Total VMs"] >= min_vms].sort_values(
                sort_by, ascending=(sort_by != "Coverage %")
            )

            # Display table
            st.dataframe(
                df_filtered.style.format(
                    {"Total VMs": "{:,}", "Labeled VMs": "{:,}", "Unlabeled VMs": "{:,}", "Coverage %": "{:.1f}%"}
                ),
                hide_index=True,
                width="stretch",
            )
