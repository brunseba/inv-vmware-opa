...
def render(db_url: str):
    ...
    try:
        ...
        with tab1:
            colored_header(
                label="Full Database Backup", description="Create a complete backup of all data", color_name="green-70"
            )

            st.info(
                "💡 This creates a complete backup of the entire database including all VMs, labels, and assignments."
            )
                ...
                if db_backups:
                    for backup_file in db_backups[:10]:  # Show last 10
                        with st.expander(f"📄 {backup_file.name}"):
                            ...
                            with col3, open(backup_file, "rb") as f:
                                st.download_button(
                                    label="⬇️ Download",
                                    data=f.read(),
        ...
        # =====================================================================
        with tab2:
            colored_header(label="Restore Database", description="Restore from a backup file", color_name="red-70")

            st.error(
                "⚠️ **WARNING:** This will replace the entire database with the backup. All current data will be overwritten!"
            )
