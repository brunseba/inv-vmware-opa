...
from src.models import Base, MigrationTarget, PlatformType
...
def render_targets_list(service: MigrationScenarioService, session):
    ...
    if selected_target_id:
        ...
        with col1:
            ...
            st.metric(
                "📉 Compression", f"{(1-compression)*100:.0f}% savings", help=f"Compression ratio: {compression:.2f}"
            )

        with col2:
            dedup = getattr(target, "dedup_ratio", 0.8)
            st.metric("📉 Deduplication", f"{(1-dedup)*100:.0f}% savings", help=f"Dedup ratio: {dedup:.2f}")

        with col3:
            change_rate = getattr(target, "change_rate_percent", 0.10)
            st.metric("🔄 Change Rate", f"{change_rate*100:.0f}%", help="Data change during migration")

        with col4:
            delta_syncs = getattr(target, "delta_sync_count", 2)
            st.metric("🔁 Delta Syncs", f"{delta_syncs}x", help="Number of synchronizations")
    ...
    with st.form("add_target_form"):
        ...
        with col1:
            ...
            platform_type = st.selectbox(
                "Platform Type*", options=[p.value for p in PlatformType], format_func=lambda x: x.upper()
            )

        with col2:
            region = st.text_input("Region", placeholder="e.g., us-east-1, westeurope")
            support_level = st.selectbox("Support Level", options=["24/7", "Business Hours", "Best Effort"])
        ...
        with col1:
            bandwidth_mbps = st.number_input("Bandwidth (Mbps)*", min_value=10, max_value=100000, value=1000, step=100)

        with col2:
            network_efficiency = st.slider(
                "Network Efficiency (%)", min_value=50, max_value=100, value=80, step=5, help="Typical: 70-80%"
            )
            # Convert percentage to decimal for storage
            network_efficiency = network_efficiency / 100.0

        with col3:
            max_parallel = st.number_input("Max Parallel Migrations", min_value=1, max_value=100, value=10)
        ...
        with col1:
            compute_cost = st.number_input(
                "Compute ($/vCPU/hour)", min_value=0.0, max_value=10.0, value=0.05, step=0.01, format="%.4"
            )
            memory_cost = st.number_input(
                "Memory ($/GB/hour)", min_value=0.0, max_value=1.0, value=0.01, step=0.001, format="%.4"
            )

        with col2:
            storage_cost = st.number_input(
                "Storage ($/GB/month)", min_value=0.0, max_value=1.0, value=0.001, step=0.0001, format="%.4"
            )
            network_ingress_cost = st.number_input(
                "Ingress ($/GB)", min_value=0.0, max_value=1.0, value=0.0, step=0.01, format="%.4"
            )

        with col3:
            network_egress_cost = st.number_input(
                "Egress ($/GB)", min_value=0.0, max_value=1.0, value=0.09, step=0.01, format="%.4"
            )
            sla_uptime = st.number_input(
                "SLA Uptime (%)", min_value=90.0, max_value=100.0, value=99.9, step=0.1, format="%.2"
            )
...
def render_edit_target(service: MigrationScenarioService, session):
    ...
    selected_target_id = st.selectbox(
        ...
        format_func=lambda x: next(
            (f"{t.name} ({t.platform_type.value.upper()})" for t in all_targets if t.id == x), "Unknown"
        ),
    ...
    with st.form("edit_target_form"):
        ...
        with col2:
            ...
            support_level = st.selectbox(
                ...
                index=(
                    ["24/7", "Business Hours", "Best Effort"].index(target.support_level)
                    if target.support_level in ["24/7", "Business Hours", "Best Effort"]
                    else 0
                ),
            )
        ...
        with col1:
            bandwidth_mbps = st.number_input(
                "Bandwidth (Mbps)*", min_value=10, max_value=100000, value=target.bandwidth_mbps, step=100
            )
        ...
        with col3:
            max_parallel = st.number_input(
                "Max Parallel Migrations", min_value=1, max_value=100, value=target.max_parallel_migrations
            )
