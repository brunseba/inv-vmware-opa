...
def render(db_url: str):
    ...
    try:
        ...
        colored_header(
            label="Report Configuration", description="Customize your report settings", color_name="violet-70"
        )
        ...
        with st.expander("⚙️ Advanced Options"):
            ...
            with col1:
                ...
                chart_dpi = st.slider(
                    "Chart Quality (DPI)", 100, 300, 150, 25, help="Higher = better quality but larger file"
                )
        ...
        colored_header(
            label="Report Contents", description="This report includes the following sections", color_name="green-70"
        )
        ...
        with tab3:
            ...
            fig.update_layout(
                height=200, margin=dict(l=0, r=0, t=0, b=0), showlegend=False, xaxis_title="", yaxis_title="VM Count"
            )
        ...
        # Export section
        colored_header(
            label="Generate Report", description="Ready to generate your customized PDF report", color_name="orange-70"
        )
        ...
        style_metric_cards(
            background_color="#1f1f1", border_left_color="#ff8c00", border_color="#2e2e2e", box_shadow="#1f1f1f"
        )
                ...
                with st.spinner("Generating PDF report... This may take a moment."):
                    try:
                        ...
                        report = VMwareInventoryReport(
                            db_url, include_charts=include_charts, extended=is_extended, chart_dpi=chart_dpi
                        )
                        ...
                        filename = f"vmware_inventory_report_{timestamp}.pd"
        ...
        if st.session_state.report_history:
            colored_header(
                label="Recent Reports", description="Previously generated reports in this session", color_name="blue-70"
            )
        ...
        with st.expander("⚡ Quick Actions"):
            ...
            with col2:
                if st.button("📋 Quick Summary\n(Tables Only)", width="stretch"):
                    st.session_state.pdf_config = {"include_charts": False, "detailed_tables": False, "max_items": 5}
                    st.success("Quick summary preset loaded!")

            with col3:
                if st.button("🖨️ Print Quality\n(High DPI)", width="stretch"):
                    st.session_state.pdf_config = {"include_charts": True, "detailed_tables": True, "chart_dpi": 300}
