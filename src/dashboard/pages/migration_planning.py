...
def render(db_url: str):
    ...
    try:
        ...
        if st.session_state.get("quick_fixed_time_override") and fixed_time_hours != st.session_state.get(
            "quick_fixed_time_override"
        ):
        ...
        bandwidth_preset = st.sidebar.selectbox(
            ...
            index=(
                bandwidth_presets.index(default_bandwidth_preset)
                if default_bandwidth_preset in bandwidth_presets
                else 1
            ),
        ...
        # Clear override if sidebar value changed
        if st.session_state.get("quick_bandwidth_override") and bandwidth_preset != st.session_state.get(
            "quick_bandwidth_override"
        ):
        ...
        if bandwidth_preset == "Custom":
            ...
            bandwidth_mbps = st.sidebar.number_input(
                "Custom Bandwidth (Mbps)", min_value=1, max_value=100000, value=default_custom_bandwidth, step=100
            )
        else:
            bandwidth_map = {"100 Mbps": 100, "1 Gbps": 1000, "10 Gbps": 10000, "25 Gbps": 25000}
        ...
        # Clear override if sidebar value changed
        if st.session_state.get("quick_parallel_override") and parallel_vms != st.session_state.get(
            "quick_parallel_override"
        ):
        ...
        # Clear override if sidebar value changed
        if st.session_state.get("quick_window_override") and maintenance_window_hours != st.session_state.get(
            "quick_window_override"
        ):
        ...
        colored_header(
            label="VM Selection", description="Select VMs to include in migration planning", color_name="green-70"
        )
        ...
        with st.expander("⚡ Configuration & Quick Adjustments", expanded=True):
            ...
            st.caption(
                "💡 Quickly adjust migration parameters without scrolling to the sidebar. Changes apply immediately when you click Apply."
            )
            st.info(
                """
            **How to use:**
            ...
            with col4:
                ...
                current_bandwidth_str = (
                    f"{bandwidth_mbps} Mbps" if bandwidth_mbps not in [100, 1000, 10000, 25000] else bandwidth_preset
                )
            ...
            with col3:
                st.metric("Daily Capacity", f"{maintenance_window_hours}h", help="Available migration hours per day")
        ...
        if selection_strategy == "Infrastructure-based":
            ...
            with col1:
                ...
                selected_dc = st.selectbox("Datacenter", options=["All"] + datacenters, index=0)
            ...
            with col1:
                ...
                if folder_level == "Full Path":
                    ...
                    available_folders = sorted(
                        set("/".join(f.split("/")[:level]) for f in all_folders if f.split("/")[:level])
                    )

            with col2:
                # Regex filter option
                use_regex = st.checkbox(
                    "Use Regex Filter", value=False, help="Enable regex pattern matching for folder names"
                )
            ...
            selected_folders = st.multiselect(
                ...
                default=(
                    available_folders_display[: min(3, len(available_folders_display))]
                    if not use_regex or not regex_pattern
                    else []
                ),
            ...
            query = session.query(VirtualMachine).filter(
                VirtualMachine.in_use_mib.isnot(None), VirtualMachine.folder.isnot(None)
            )
        ...
        for vm in vms:
            ...
            replication_time = calculate_replication_time(
                storage_gib, bandwidth_mbps / parallel_vms, network_efficiency  # Bandwidth per VM when parallel
            )
        ...
        add_vertical_space(1)
        colored_header(
            label="Migration Summary", description="Estimated time and resource requirements", color_name="orange-70"
        )
        ...
        if current_batch:
            migration_batches.append(
                {"batch": batch_num, "vms": current_batch, "duration": max(vm["Total_Hours"] for vm in current_batch)}
            )
        ...
        # Visualizations
        colored_header(
            label="Migration Analysis", description="Visual breakdown of migration plan", color_name="violet-70"
        )

        if selection_strategy == "Folder-based":
            tab1, tab2, tab3, tab4, tab5 = st.tabs(
                ["📊 Overview", "📅 Timeline", "💾 Storage", "⚡ Performance", "📁 Folders"]
            )
        else:
        ...
        with tab1:
            ...
            with col1:
                ...
                fig.update_layout(title="Migration Time Comparison", barmode="stack", yaxis_title="Hours", height=400)
        ...
        with tab2:
            ...
            if not df_gantt.empty:
                ...
                if len(gantt_data) > 50:
                    st.info(
                        f"Showing first 50 of {len(gantt_data)} VMs. Download full migration plan for complete timeline."
                    )
        ...
        with tab3:
            ...
            with col1:
                ...
                for batch in migration_batches:
                    ...
                    batch_storage_data.append({"Batch": f"Batch {batch['batch']}", "Storage_GiB": batch_storage})
            ...
            fig.update_layout(
                xaxis_title="Storage Size (GiB)", yaxis_title="Number of VMs", showlegend=False, height=350
            )
            fig = ThemeManager.apply_chart_theme(fig)
            ...
            with col2:
                st.metric(
                    "Average VM Size", f"{df['Storage_GiB'].mean():.1f} GiB", help="Mean storage size across all VMs"
                )
            with col3:
                st.metric("Smallest VM", f"{df['Storage_GiB'].min():.1f} GiB", help="Smallest VM by storage size")
            with col4:
                st.metric("Largest VM", f"{df['Storage_GiB'].max():.1f} GiB", help="Largest VM by storage size")
            ...
            with col2:
                ...
                fig = px.scatter(
                    ...
                    labels={"Storage_GiB": "Storage (GiB)", "Replication_Hours": "Replication Time (hours)"},
                )
        ...
        with tab4:
            ...
            fig.add_hline(
                y=effective_bandwidth_mbps, line_dash="dash", line_color="orange", annotation_text="Effective Bandwidth"
            )

            # Add line for total bandwidth
            fig.add_hline(y=bandwidth_mbps, line_dash="dot", line_color="red", annotation_text="Total Bandwidth")
            ...
            with col2:
                avg_util = (
                    (sum(bandwidth_usage) / len(bandwidth_usage) / effective_bandwidth_mbps * 100)
                    if bandwidth_usage and effective_bandwidth_mbps > 0
                    else 0
                )
                st.metric("Avg Utilization", f"{avg_util:.1f}%", help="Average utilization of effective bandwidth")
            with col3:
                st.metric("Data Transferred", f"{total_storage:,.1f} GiB", help="Total storage to be migrated")
        ...
        if selection_strategy == "Folder-based":
            with tab5:
                ...
                folder_summary = (
                    df.groupby("Folder")
                    .agg({"VM": "count", "Storage_GiB": "sum", "Total_Hours": "sum", "Batch": lambda x: x.nunique()})
                    .reset_index()
                )
                ...
                st.dataframe(
                    folder_summary.style.format(
                        {"VM_Count": "{:,.0f}", "Total_Storage": "{:,.1f}", "Total_Time": "{:.2f}", "Batches": "{:.0f}"}
                    ),
                    height=300,
                    width="stretch",
                )
        ...
        colored_header(
            label="Migration Plan Details", description="Detailed VM-by-VM migration plan", color_name="red-70"
        )
        ...
        with col2:
            ...
            with pd.ExcelWriter(excel_buffer, engine="xlsxwriter") as writer:
                ...
                chart_sheet.write(
                    "A2", "Charts from web interface tabs | Timeline data available in Timeline sheet", note_format
                )
                ...
                comparison_sheet.write_row(
                    "A3", ["Parallel", replication_parallel, len(migration_batches) * fixed_time_hours]
                )
                ...
                chart_sheet.write(
                    "A26", "Timeline data with Start/End times available in Timeline sheet tab", note_format
                )
                ...
                chart_sheet.write(
                    "A52",
                    "Note: Storage vs Replication Time scatter plot data available in Storage Analysis sheet",
                    note_format,
                )
                ...
                chart_sheet.write(
                    "A76", "Note: Bandwidth utilization line chart represents network usage over time", note_format
                )
