...
def render(db_url: str):
    ...
    try:
        ...
        # Comparison mode
        comparison_mode = st.radio("Compare by:", ["Datacenters", "Clusters", "Hosts"], horizontal=True)
...
def _compare_datacenters(session):
    ...
    selected_dcs = st.multiselect(
        "Select datacenters to compare (select 2-5):", datacenters, default=datacenters[: min(3, len(datacenters))]
    )
    ...
    for metric in metrics:
        ...
        if max_val > 0:
            for dc in selected_dcs:
                ...
                normalized_data.append({"Datacenter": dc, "Metric": metric, "Normalized Value": (val / max_val) * 100})
...
def _compare_clusters(session):
    ...
    selected_clusters = st.multiselect(
        "Select clusters to compare (select 2-5):", clusters, default=clusters[: min(3, len(clusters))]
    )
...
def _compare_hosts(session):
    ...
    selected_hosts = st.multiselect(
        "Select hosts to compare (select 2-10):", hosts, default=hosts[: min(5, len(hosts))]
    )
    ...
    fig.add_trace(
        go.Bar(name="Total VMs", x=df_comparison["Host"], y=df_comparison["Total VMs"], marker_color="lightblue")
    )

    fig.add_trace(
        go.Bar(name="Powered On", x=df_comparison["Host"], y=df_comparison["Powered On"], marker_color="green")
    )

    fig.update_layout(title="Host VM Distribution", barmode="group", xaxis_tickangle=-45)
