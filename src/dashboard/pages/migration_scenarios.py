...
def render(db_url: str):
    ...
    try:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["📋 Scenarios List", "➕ Create Scenario", "✏️ Edit Scenario", "📊 Compare Scenarios", "🌊 Migration Waves"]
        )
                            ...
                            for scenario_id in selected_recalc_ids:
                                ...
                                if scenario:
                                    ...
                                    if vms:
                                        ...
                                        scenario.estimated_runtime_cost_monthly = cost_breakdown["runtime_monthly"][
                                            "total"
                                        ]
    ...
    if selected_scenario_id:
        ...
        try:
            ...
            if vms and target:
                ...
                with col4:
                    st.metric("🎯 Total", f"{duration_detail.get('total_hours', 0):.1f}h", help="Total migration time")
                ...
                with col1:
                    ...
                    st.metric("📊 Original Data", f"{original_tb:.2f} TB", help="Total VM storage before optimization")

                with col2:
                    effective_tb = duration_detail.get("effective_data_tb", 0)
                    st.metric("💾 Transferred", f"{effective_tb:.2f} TB", help="Actual data transferred")

                with col3:
                    compression_pct = duration_detail.get("compression_savings_percent", 0)
                    st.metric("📉 Compression", f"{compression_pct:.0f}%", help="Compression savings")

                with col4:
                    dedup_pct = duration_detail.get("dedup_savings_percent", 0)
                    st.metric("📉 Dedup", f"{dedup_pct:.0f}%", help="Deduplication savings")
        ...
        with col2:
            ...
            st.metric("🔄 Runtime Cost (Monthly)", f"${runtime_cost:,.2f}", help="Ongoing monthly operational costs")
            ...
            with col1:
                if scenario.migration_cost_breakdown:
                    ...
                    if migration_data:
                        ...
                        fig1 = px.pie(mig_df, values="Cost", names="Category", title="Migration Costs")
            ...
            with col2:
                if scenario.runtime_cost_breakdown:
                    ...
                    if runtime_data:
                        ...
                        fig2 = px.pie(run_df, values="Cost", names="Category", title="Monthly Runtime Costs")
                        fig = ThemeManager.apply_chart_theme(fig)
...
def render_create_scenario(service: MigrationScenarioService, session):
    ...
    with st.form("create_scenario_form"):
        ...
        with col2:
            ...
            target_id = st.selectbox(
                ...
                format_func=lambda x: next(
                    (f"{t.name} ({t.platform_type.value.upper()})" for t in targets if t.id == x), "Unknown"
                ),
            )
        ...
        selection_type = st.radio(
            "Selection Method", options=["By Datacenter/Cluster", "By Folder", "By Label", "By VM IDs"], horizontal=True
        )
        ...
        if selection_type == "By Datacenter/Cluster":
            ...
            if not labels:
                ...
                with col1:
                    selected_keys = st.multiselect(
                        "Label Keys", options=label_keys, help="Select label keys to filter by"
                    )
                ...
                with col2:
                    if selected_keys:
                        ...
                        available_values = [
                            f"{label.key}:{label.value}" for label in labels if label.key in selected_keys
                        ]
                        selected_label_pairs = st.multiselect(
                            "Label Values", options=available_values, help="Select specific key:value pairs"
                        )
            ...
            vm_ids_input = st.text_area(
                "VM IDs (comma-separated)", placeholder="1,2,3,4,5", help="Enter VM IDs separated by commas"
            )
            if vm_ids_input:
                try:
        ...
        with col2:
            parallel_migrations = st.number_input(
                "Parallel Migrations", min_value=1, max_value=50, value=5, help="Number of VMs migrating simultaneously"
            )
    ...
    with st.form("edit_scenario_form"):
        ...
        with col2:
            target_id = st.selectbox(
                ...
                index=(
                    [t.id for t in targets].index(scenario.target_id)
                    if scenario.target_id in [t.id for t in targets]
                    else 0
                ),
            )
                    ...
                    if recalculate:
                        with st.spinner("Recalculating estimates..."):
                            ...
                            if vms:
                                # Recalculate estimates
                                cost_breakdown = service.calculate_migration_cost(
                                    vms, target, 30, MigrationStrategy(strategy)
                                )
                                duration = service.calculate_migration_duration(
                                    vms, target, 5, MigrationStrategy(strategy)
                                )
                                risk_level, risk_factors = service.assess_risk_level(
                                    vms, target, MigrationStrategy(strategy)
                                )
                                recommendation_score = service.calculate_recommendation_score(
                                    cost_breakdown, duration, risk_level, target
                                )
...
def render_compare_scenarios(service: MigrationScenarioService):
    ...
    with col1:
        # Cost comparison
        fig_cost = px.bar(comparison_df, x="Scenario", y="Total Cost ($)", color="Platform", title="Cost Comparison")
        fig = ThemeManager.apply_chart_theme(fig)
    ...
    with col2:
        # Duration comparison
        fig_duration = px.bar(
            comparison_df, x="Scenario", y="Duration (days)", color="Risk Level", title="Duration Comparison"
        )
        fig = ThemeManager.apply_chart_theme(fig)
    ...
    if len(selected_ids) <= 5:  # Only show radar for <=5 scenarios
        ...
        fig_radar.update_layout(
            polar=dict(radialaxis=dict(visible=True, range=[0, 100])), title="Multi-Criteria Comparison"
        )
        fig = ThemeManager.apply_chart_theme(fig)
...
def render_migration_waves(service: MigrationScenarioService, session):
    ...
    existing_waves = (
        session.query(MigrationWave)
        .filter(MigrationWave.scenario_id == scenario.id)
        .order_by(MigrationWave.wave_number)
        .all()
    )
        ...
        with st.form("generate_waves_form"):
            ...
            with col1:
                wave_size = st.number_input(
                    "VMs per Wave", min_value=1, max_value=100, value=10, help="Number of VMs to migrate in each wave"
                )
                ...
                try:
                    with st.spinner("Generating migration waves..."):
                        waves = service.generate_migration_waves(
                            scenario_id=scenario.id, wave_size=wave_size, strategy=wave_strategy
                        )
