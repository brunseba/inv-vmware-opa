...
def render(db_url: str):
    ...
    colored_header(
        label="🔍 VM Search", description="Advanced regex pattern matching for VM names", color_name="blue-70"
    )
    ...
    try:
        ...
        # Search input
        colored_header(
            label="Search Pattern", description="Enter a regex pattern to search VM names", color_name="green-70"
        )
        ...
        # Filters
        colored_header(label="Filters", description="Narrow down search results", color_name="orange-70")
        ...
        with col3:
            power_states = ["All", "poweredOn", "poweredOf"]
        ...
        if search_button or (pattern and len(pattern) >= 2):
            ...
            colored_header(
                ...
                description=f"Found {len(matching_vms):,} matching VMs"
                + (f" (showing {limit})" if len(matching_vms) > limit else ""),
            ...
            st.dataframe(df_results.style.format({"Memory (GB)": "{:.1f}"}), width="stretch", hide_index=True)
