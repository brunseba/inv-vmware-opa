    ...
    try:
        ...
        with col3:
            sort_by = st.selectbox(
                "Sort by", ["VMs", "Total_CPUs", "Total_Memory_GB", "Total_Provisioned_GB", "Total_In_Use_GB", "Folder"]
            )
        ...
        with col_label1:
            ...
            selected_label_key = st.selectbox(
                "Filter by Label Key", ["All"] + label_keys, help="Filter folders by label key"
            )
        ...
        if aggregate_level != "Full Path":
            ...
            df_folders["Aggregated_Folder"] = df_folders["Folder"].apply(lambda x: _aggregate_folder_path(x, level))

            # Re-aggregate by new folder paths
            df_folders = (
                df_folders.groupby("Aggregated_Folder")
                .agg(
                    {
                        "VMs": "sum",
                        "Total_CPUs": "sum",
            ...
            st.info(
                f"📂 Viewing folders aggregated at **{aggregate_level}** (folders grouped by first {aggregate_level.split()[1]} level(s))"
            )
        ...
        # Summary metrics
        colored_header(label="Folder Summary", description="Key folder metrics and statistics", color_name="orange-70")
        ...
        with col5:
            folders_with_null = (
                session.query(func.count(VirtualMachine.id)).filter(VirtualMachine.folder.is_(None)).scalar() or 0
            )
        ...
        if selected_label_key != "All":
            ...
            if selected_label_value and selected_label_value != "All":
                ...
                if label:
                    labeled_folders = (
                        session.query(FolderLabel.folder_path).filter(FolderLabel.label_id == label.id).all()
                    )
                ...
                if label_ids:
                    labeled_folders = (
                        session.query(FolderLabel.folder_path)
                        .filter(FolderLabel.label_id.in_(label_ids))
                        .distinct()
                        .all()
                    )
                    labeled_folders = [f[0] for f in labeled_folders]
            ...
            if len(df_folders) > 0:
                st.info(
                    f"🏷️ Showing {len(df_folders)} folders with label: {selected_label_key}"
                    + (f"={selected_label_value}" if selected_label_value and selected_label_value != "All" else "")
                )
            else:
                st.warning(
                    f"No folders found with label: {selected_label_key}"
                    + (f"={selected_label_value}" if selected_label_value and selected_label_value != "All" else "")
                )
        ...
        st.download_button(
            label="⬇️ Export Folder Analysis CSV", data=csv_data, file_name="folder_analysis.csv", mime="text/csv"
        )
        ...
        # Visualizations
        colored_header(
            label="Folder Visualizations", description="Interactive charts and analytics", color_name="blue-70"
        )

        viz_tab1, viz_tab2, viz_tab3, viz_tab4 = st.tabs(
            ["📊 Distribution", "🎯 Resources", "💾 Storage", "🗂️ Hierarchy"]
        )
        ...
        with viz_tab2:
            ...
            with col1:
                ...
                fig = go.Figure()
                fig.add_trace(
                    go.Bar(name="vCPUs", x=df_top["Folder"], y=df_top["Total_CPUs"], marker_color="lightblue")
                )
                fig.add_trace(
                    go.Bar(
                        name="Memory (GB)", x=df_top["Folder"], y=df_top["Total_Memory_GB"], marker_color="lightcoral"
                    )
                )

                fig.update_layout(
                    title="Top 10 Folders - Resource Allocation", barmode="group", xaxis_tickangle=-45, height=500
                )
                fig = ThemeManager.apply_chart_theme(fig)
            ...
            with col2:
                ...
                fig.update_layout(xaxis_title="Avg vCPUs per VM", yaxis_title="Avg Memory (GB) per VM", height=500)
                fig = ThemeManager.apply_chart_theme(fig)
        ...
        with viz_tab3:
            ...
            with col2:
                ...
                fig.add_trace(
                    go.Bar(name="In Use", x=df_top["Folder"], y=df_top["Total_In_Use_GB"], marker_color="lightseagreen")
                )
            ...
            with col1:
                ...
                df_top["Storage_Efficiency_%"] = (
                    df_top["Total_In_Use_GB"] / df_top["Total_Provisioned_GB"] * 100
                ).round(1)
                ...
                fig = px.scatter(
                    ...
                    color_continuous_scale="Viridis",
                    labels={
                        "Storage_Efficiency_%": "Efficiency (%)",
                        "Total_Provisioned_GB": "Provisioned Storage (GB)",
                    },
                )
            ...
            with col2:
                ...
                fig.update_layout(showlegend=False, yaxis={"categoryorder": "total ascending"}, height=450)
                fig = ThemeManager.apply_chart_theme(fig)
        ...
        colored_header(
            label="🔍 Folder Details", description="Drill down into specific folder contents", color_name="red-70"
        )

        selected_folder = st.selectbox("Select a folder to inspect:", df_filtered["Folder"].tolist())
        ...
        if selected_folder:
            ...
            folder_vms = session.query(VirtualMachine).filter(VirtualMachine.folder == selected_folder).all()
