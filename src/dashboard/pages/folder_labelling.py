...
def render(db_url: str):
    ...
    try:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["📋 Label Definitions", "📁 Folder Labels", "🖥️ VM Labels", "🔍 Search by Label", "⚙️ Management"]
        )
        ...
        with tab1:
            colored_header(
                label="Label Definitions", description="Create and manage label definitions", color_name="green-70"
            )
                ...
                for key in sorted(labels_by_key.keys()):
                    with st.expander(f"🏷️ {key} ({len(labels_by_key[key])} values)"):
                        for label in labels_by_key[key]:
                            ...
                            with col1:
                                st.markdown(
                                    _render_label_badge(label.key, label.value, label.color), unsafe_allow_html=True
                                )
        ...
        with tab2:
            ...
            with filter_col1:
                # Get unique clusters
                clusters = (
                    session.query(VirtualMachine.cluster)
                    .filter(VirtualMachine.cluster.isnot(None))
                    .distinct()
                    .order_by(VirtualMachine.cluster)
                    .all()
                )
                cluster_list = [c[0] for c in clusters]
                selected_cluster_filter = st.selectbox(
                    "Filter by Cluster", ["All Clusters"] + cluster_list, key="cluster_filter"
                )

            with filter_col2:
                # Get unique datacenters
                datacenters = (
                    session.query(VirtualMachine.datacenter)
                    .filter(VirtualMachine.datacenter.isnot(None))
                    .distinct()
                    .order_by(VirtualMachine.datacenter)
                    .all()
                )
                datacenter_list = [d[0] for d in datacenters]
                selected_dc_filter = st.selectbox(
                    "Filter by Datacenter", ["All Datacenters"] + datacenter_list, key="dc_filter"
                )
            ...
            if selected_cluster_filter != "All Clusters" or selected_dc_filter != "All Datacenters":
                ...
                vm_query = session.query(VirtualMachine.folder).filter(VirtualMachine.folder.isnot(None))
            ...
            with st.expander("📦 Bulk Assign Label to All Folders", expanded=False):
                ...
                with col2:
                    bulk_inherit_subfolders = st.checkbox(
                        "Apply to subfolders", value=False, key="bulk_inherit_subfolders"
                    )
                with col3:
            ...
            with st.expander("➕ Assign Label to Single Folder", expanded=False):
                ...
                with col1:
                    selected_folder = st.selectbox(
                        "Select Folder", folders if folders else ["No folders match filter"], key="folder_assign"
                    )
            ...
            folder_search = st.text_input(
                "🔍 Search folders by name", placeholder="Type to search...", key="folder_search"
            )
                ...
                if folder_labels:
                    with st.expander(f"📁 {folder} ({len(folder_labels)} label(s))"):
                        ...
                        for lbl in folder_labels:
                            ...
                            with col1:
                                st.markdown(
                                    _render_label_badge(lbl["key"], lbl["value"], lbl["color"]), unsafe_allow_html=True
                                )
        ...
        # =====================================================================
        with tab3:
            colored_header(label="VM Labels", description="Assign labels directly to VMs", color_name="violet-70")
                ...
                if vm_search and len(vm_search) >= 2:
                    try:
                        vms = (
                            ...
                            .filter(VirtualMachine.vm.ilike(f"%{vm_search}%"))
                            .limit(20)
                            .all()
                        )
                    except Exception as e:
                            ...
                            with st.expander(f"🖥️ {vm.vm}"):
                                ...
                                if vm_labels:
                                    ...
                                    for lbl in vm_labels:
                                        ...
                                        with col1:
                                            st.markdown(
                                                _render_label_badge(lbl["key"], lbl["value"], lbl["color"]),
                                ...
                                with col3:
                                    if st.button("Assign", key=f"assign_vm_{vm.id}", type="primary"):
                                        if vm_label_key and vm_label_value:
                                            try:
                                                label = label_service.get_label_by_key_value(
                                                ...
                                                if (
                                                    ...
                                                    or "already assigned" in error_msg.lower()
                                                ):
                ...
                with col1:
                    if filter_type == "OS Family":
                        category = st.selectbox(
                            "OS Family", ["windows", "linux", "unix", "other"], key="os_family_select"
                        )
                        ...
                        try:
                            os_list = (
                                session.query(VirtualMachine.os_config)
                        ...
                        if os_options:
                            ...
                            match_mode = st.radio(
                                "Match Mode", ["Exact Match", "Contains Pattern"], key="os_match_mode", horizontal=True
                            )

                            if match_mode == "Exact Match":
                                category = st.selectbox("Select OS", os_options, key="specific_os_select")
                                ...
                                if category:
                                    matching_os = [
                                        os for os in os_options if category.replace("%", "").lower() in os.lower()
                                    ]
                                    if matching_os:
                        ...
                        category = st.selectbox(
                            "Resource Category", ["small", "medium", "large", "xlarge"], key="resource_category_select"
                        )
                        size_descriptions = {
                        ...
                        category = st.selectbox(
                            "Network Complexity", ["simple", "standard", "complex"], key="network_complexity_select"
                        )
                        net_descriptions = {
                        ...
                        category = st.selectbox(
                            "Storage Complexity", ["simple", "standard", "complex"], key="storage_complexity_select"
                        )
                    ...
                    try:
                        ...
                        if filter_type == "OS Family":
                            ...
                            if category:
                                ...
                                if (
                                    "os_match_mode" in st.session_state
                                    and st.session_state.os_match_mode == "Exact Match"
                                ):
                                    # Exact match
                                    matching_vms = (
                                        session.query(VirtualMachine).filter(VirtualMachine.os_config == category).all()
                                    )
                                else:
                        ...
                        if "batch_vm_ids" in st.session_state and st.session_state["batch_vm_ids"]:
                            try:
                                ...
                                if label:
                                    ...
                                    with st.spinner(
                                        f"Assigning {batch_label_key}={batch_label_value} to {len(vm_ids)} VMs..."
                                    ):
                                    ...
                                    st.info(
                                        f"📋 **Results:** {successful} assigned, {failed} skipped (already assigned or error)"
                                    )
        ...
        # =====================================================================
        with tab4:
            colored_header(label="Search by Label", description="Find VMs and folders by labels", color_name="red-70")
        ...
        with tab5:
            colored_header(
                label="Label Management", description="Maintenance and synchronization operations", color_name="gray-70"
            )
            ...
            with col2:
                ...
                # Count assignments
                vm_label_count = (
                    session.query(VirtualMachine).join("virtual_machines").count()
                    if hasattr(VirtualMachine, "labels")
                    else 0
                )

                st.caption("📊 Active in system")
            ...
            with col1:
                ...
                if st.button("💾 Create Backup", type="primary", key="create_backup"):
                    try:
                        ...
                        st.info(
                            f"📊 Labels: {stats['labels']}, VM Assignments: {stats['vm_assignments']}, Folder Assignments: {stats['folder_assignments']}"
                        )
            ...
            with col2:
                ...
                # File uploader
                uploaded_file = st.file_uploader("Choose backup file", type=["json"], key="restore_file")
