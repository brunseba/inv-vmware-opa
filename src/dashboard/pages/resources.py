...
def render(db_url: str):
    ...
    colored_header(
        label="💾 Resource Analysis", description="CPU, Memory, and Storage consumption metrics", color_name="blue-70"
    )
    ...
    try:
        ...
        # Summary metrics
        colored_header(
            label="Resource Summary", description=f"Analyzing {len(vms):,} virtual machines", color_name="green-70"
        )
        ...
        # CPU Distribution
        colored_header(
            label="Resource Distribution", description="CPU and memory allocation across VMs", color_name="orange-70"
        )
        ...
        with col1:
            ...
            if cpu_data:
                ...
                fig = px.histogram(df_cpu, x="vCPUs", nbins=20, color_discrete_sequence=["#1f77b4"])
                fig.update_layout(xaxis_title="Number of vCPUs", yaxis_title="VM Count", showlegend=False)
        ...
        with col2:
            ...
            if memory_data:
                ...
                fig = px.histogram(df_mem, x="Memory_GB", nbins=20, color_discrete_sequence=["#ff7f0e"])
                fig.update_layout(xaxis_title="Memory (GB)", yaxis_title="VM Count", showlegend=False)
                fig = ThemeManager.apply_chart_theme(fig)
        ...
        with col1:
            ...
            df_top_cpu = pd.DataFrame([{"VM": vm.vm[:30], "vCPUs": vm.cpus} for vm in top_cpu_vms if vm.cpus])
            if not df_top_cpu.empty:
                fig = px.bar(
                    df_top_cpu, x="vCPUs", y="VM", orientation="h", color="vCPUs", color_continuous_scale="Blues"
                )
        ...
        with col2:
            ...
            df_top_mem = pd.DataFrame(
                [{"VM": vm.vm[:30], "Memory_GB": vm.memory / 1024} for vm in top_mem_vms if vm.memory]
            )
            if not df_top_mem.empty:
                fig = px.bar(
                    df_top_mem, x="Memory_GB", y="VM", orientation="h", color="Memory_GB", color_continuous_scale="Reds"
                )
                fig.update_layout(showlegend=False, yaxis={"categoryorder": "total ascending"})
                fig = ThemeManager.apply_chart_theme(fig)
        ...
        # Storage Analysis
        colored_header(
            label="Storage Analysis", description="Storage provisioning and utilization metrics", color_name="violet-70"
        )
        col1, col2, col3 = st.columns(3)
        ...
        fig = px.bar(
            storage_df, x="Type", y="Size_GB", color="Type", color_discrete_sequence=px.colors.qualitative.Set2
        )
