...
from src.loader import get_sheet_names, load_excel_to_db
            ...
            try:
                ...
                with col1:
                    ...
                    selected_sheet = st.selectbox(
                        "Select Sheet", options=sheet_names, help="Choose which sheet to import"
                    )

                with col2:
                    # Clear existing data option
                    clear_existing = st.checkbox(
                        "Clear Existing Data", value=True, help="Remove all existing data before import"
                    )
                    ...
                    with st.spinner("Importing data... This may take a few minutes for large files."):
                        try:
                            ...
                            records_loaded = load_excel_to_db(
                                tmp_path, db_url, clear_existing=clear_existing, sheet_name=selected_sheet
                            )
        ...
        try:
            ...
            try:
                ...
                if schema_info["compatible"]:
                    ...
                    st.warning(
                        f"⚠️ Schema Version: {schema_info['current_version'] or 'Unknown'} (Expected: {schema_info['expected_version']})"
                    )
            except Exception:
                pass
            ...
            if tables:
                ...
                for table_name in tables:
                    try:
                        ...
                        table_data.append({"Table": table_name, "Records": f"{count:,}"})
                    except Exception:
                        table_data.append({"Table": table_name, "Records": "Error"})
            ...
            if vm_count > 0:
                ...
                with stat_col2:
                    powered_on = session.query(VirtualMachine).filter(VirtualMachine.powerstate == "poweredOn").count()
                ...
                with st.expander("📊 Detailed Statistics"):
                    ...
                    with detail_col2:
                        st.write("**VM States:**")
                        powered_off = (
                            session.query(VirtualMachine).filter(VirtualMachine.powerstate == "poweredOf").count()
                        )
                        suspended = (
                            session.query(VirtualMachine).filter(VirtualMachine.powerstate == "suspended").count()
                        )
                ...
                if st.button("✅ Yes, Clear Data", type="primary"):
                    try:
                        ...
                        if clear_mode == "VM Data + Label Mappings (Preserve label definitions)":
                            ...
                            results.append("ℹ️ Preserved label definitions")
                            ...
                            results.append("ℹ️ Preserved all labels and mappings")

                        session.commit()
                        session.close()

                        st.success("✅ Clear operation completed!")
            ...
            if st.button("Optimize Database", type="secondary"):
                try:
                    ...
                    with engine.connect() as conn:
                        ...
                        st.info(
                            f"📊 Size before: {size_before:.2f} MB → After: {size_after:.2f} MB (Saved: {saved:.2f} MB)"
                        )
                except Exception as e:
                    st.error(f"❌ Error: {e}")
