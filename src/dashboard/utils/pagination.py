...
def paginate_query(
    query: Query, page: int = 1, page_size: int = DEFAULT_PAGE_SIZE, count_query: Query | None = None
) -> PaginatedResult:
...
def show_pagination_controls(result: PaginatedResult, key_prefix: str = "pagination") -> int:
...
def show_results_warning(total: int, config: PaginationConfig):
    ...
    if total > config.max_display:
        st.warning(
            ...
            "Use filters to narrow down your search for better performance.",
            icon="⚠️",
        )
    elif total > config.max_fetch:
        st.error(
            f"❌ Result set too large ({total:,} items exceeds limit of {config.max_fetch:,}). "
            "Please use filters to narrow down your search.",
            icon="❌",
        )
...
class PaginationHelper:
    ...
    def paginate_query(self, query: Query, _total_count: int) -> Query:
        """Apply pagination to a query.

        Args:
            query: SQLAlchemy query
            _total_count: Total number of results (unused, kept for API compatibility)
...
def paginate_with_cache(
    query_func: Callable, query_key: str, page: int = 1, page_size: int = DEFAULT_PAGE_SIZE, **query_kwargs
) -> dict:
