...
def metric_with_help(label: str, value: str, help_text: str, delta=None):
    ...
    with col2:
        st.markdown(f'<div style="margin-top: 20px;" title="{help_text}">ⓘ</div>', unsafe_allow_html=True)
...
def section_help(help_text: str):
    ...
    """
    st.markdown(
        '<span style="color: #888; font-size: 0.9em; margin-left: 10px;" ' f'title="{help_text}">ⓘ Help</span>',
        unsafe_allow_html=True,
    )
