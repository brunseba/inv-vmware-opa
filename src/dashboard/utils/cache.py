    ...
    try:
        with DatabaseManager.session_scope(db_url) as session:
            ...
            if "virtual_machines" not in inspector.get_table_names():
                return {"total": 0, "powered_on": 0, "powered_of": 0, "suspended": 0}
            ...
            suspended = (
                session.query(func.count(VirtualMachine.id)).filter(VirtualMachine.powerstate == "suspended").scalar()
                or 0
            )

            return {"total": total, "powered_on": powered_on, "powered_of": powered_off, "suspended": suspended}
...
def get_data_quality_metrics(db_url: str) -> dict[str, int]:
    ...
    with DatabaseManager.session_scope(db_url) as session:
        missing_dns = (
            session.query(func.count(VirtualMachine.id)).filter(VirtualMachine.dns_name.is_(None)).scalar() or 0
        )
        ...
        templates = session.query(func.count(VirtualMachine.id)).filter(VirtualMachine.template.is_(True)).scalar() or 0

        return {"missing_dns": missing_dns, "missing_ip": missing_ip, "templates": templates}
...
class CacheManager:
    ...
    def get_cache_info():
        ...
        return {"total_functions": len(cached_functions), "function_names": [f.__name__ for f in cached_functions]}
