...
class Label(Base):
    ...
    name: Mapped[str | None] = mapped_column(String(255), index=True)  # Combined key=value display name
    category: Mapped[str | None] = mapped_column(String(100), index=True)  # Label category for grouping
...
class VMLabel(Base):
    ...
    vm_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("virtual_machines.id", ondelete="CASCADE"), nullable=False, index=True
    )
    label_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("labels.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    # Source tracking fields (migration 004)
    label_source: Mapped[str | None] = mapped_column(
        String(50), index=True
    )  # 'naming_convention', 'manual', 'folder_inheritance'
    source_convention_id: Mapped[int | None] = mapped_column(
        Integer, index=True
    )  # FK to naming_conventions (not enforced in SQLite)
...
class SchemaVersion(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<SchemaVersion(version='{self.version}', is_current={self.is_current}, applied_at='{self.applied_at}')>"
        )
