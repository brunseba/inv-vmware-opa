...
class NamingConvention(Base):
    ...
    analyses: Mapped[list["VMNamingAnalysis"]] = relationship(
        "VMNamingAnalysis", back_populates="convention", cascade="all, delete-orphan"
    )
...
class NamingConventionField(Base):
    ...
    convention_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("naming_conventions.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class VMNamingAnalysis(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<VMNamingAnalysis(vm_name='{self.vm_name}', convention_id={self.convention_id}, valid={self.is_valid})>"
        )
