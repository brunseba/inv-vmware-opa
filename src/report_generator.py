...
class VMwareInventoryReport:
    ...
    def _create_title_page(self) -> list:
        ...
        # Date
        date_style = ParagraphStyle("DateStyle", parent=self.styles["Normal"], fontSize=12, alignment=TA_CENTER)
    ...
    def _create_executive_summary(self) -> list:
        ...
        powered_on = (
            self.session.query(func.count(VirtualMachine.id)).filter(VirtualMachine.powerstate == "poweredOn").scalar()
            or 0
        )
        ...
        data = [
            ...
            ["Powered Of", f"{powered_off:,} ({powered_off/total_vms*100:.1f}%)" if total_vms > 0 else "0"],
        ...
        ax.pie(power_data, labels=power_labels, autopct="%1.1f%%", colors=colors_pie, startangle=90)
    ...
    def _create_infrastructure_section(self) -> list:
        ...
        dc_stats = (
            self.session.query(VirtualMachine.datacenter, func.count(VirtualMachine.id).label("vm_count"))
        ...
        if dc_stats:
            ...
            if self.include_charts and len(dc_stats) > 1:
                ...
                for bar in bars:
                    width = bar.get_width()
                    ax.text(
                        width, bar.get_y() + bar.get_height() / 2, f"{int(width):,}", ha="left", va="center", fontsize=8
                    )
        ...
        if cluster_stats:
            ...
            for cluster, count, cpus, memory in cluster_stats:
                ...
                data.append([str(cluster or "Unknown"), f"{count:,}", f"{int(cpus or 0):,}", f"{memory_gb:,.0f}"])
    ...
    def _create_resource_section(self) -> list:
        ...
        cpu_stats = (
            self.session.query(VirtualMachine.cpus, func.count(VirtualMachine.id).label("count"))
            .filter(VirtualMachine.cpus.isnot(None))
            .group_by(VirtualMachine.cpus)
            .order_by(VirtualMachine.cpus)
            .all()
        )
        ...
        total_vms = (
            self.session.query(func.count(VirtualMachine.id)).filter(VirtualMachine.memory.isnot(None)).scalar() or 0
        )

        for min_mem, max_mem, label in memory_ranges:
            count = (
                self.session.query(func.count(VirtualMachine.id))
                .filter(VirtualMachine.memory >= min_mem, VirtualMachine.memory < max_mem)
                .scalar()
                or 0
            )
    ...
    def _create_folder_section(self) -> list:
        ...
        folder_stats = (
            ...
            .filter(VirtualMachine.folder.isnot(None))
            .group_by(VirtualMachine.folder)
            .order_by(func.count(VirtualMachine.id).desc())
            .limit(15)
            .all()
        )
        ...
        if folder_stats:
            ...
            for folder, count, cpus, memory in folder_stats:
                ...
                data.append([folder_name, f"{count:,}", f"{int(cpus or 0):,}", f"{memory_gb:,.0f}"])
            ...
            if self.include_charts and len(folder_stats) > 1:
                ...
                folder_names = [str(f)[:25] + "..." if len(str(f)) > 25 else str(f) for f, _, _, _ in folder_stats[:12]]
    ...
    def _create_data_quality_section(self) -> list:
        ...
        for field_name, display_name in fields_to_check:
            ...
            populated = self.session.query(func.count(VirtualMachine.id)).filter(col_attr.isnot(None)).scalar() or 0
            missing = total_vms - populated
            completeness = (populated / total_vms * 100) if total_vms > 0 else 0

            data.append([display_name, f"{populated:,}", f"{missing:,}", f"{completeness:.1f}%"])
    ...
    def _create_extended_analytics_section(self) -> list:
        ...
        vms = (
            self.session.query(VirtualMachine)
            .filter(VirtualMachine.cpus.isnot(None), VirtualMachine.memory.isnot(None))
            .all()
        )
        ...
        if vms:
            ...
            df_vms = pd.DataFrame(
                [
                    {"CPUs": vm.cpus, "Memory_GB": vm.memory / 1024, "PowerState": vm.powerstate or "Unknown"}
                    for vm in vms
                    if vm.cpus and vm.memory
                ]
            )
            ...
            ax1.scatter(powered_on["CPUs"], powered_on["Memory_GB"], alpha=0.6, s=50, c="#2ecc71", label="Powered On")
            ax1.scatter(powered_off["CPUs"], powered_off["Memory_GB"], alpha=0.6, s=50, c="#e74c3c", label="Powered Of")
            ...
            df_vms["Config"] = (
                df_vms["CPUs"].astype(str) + " CPU / " + df_vms["Memory_GB"].round(0).astype(int).astype(str) + " GB"
            )
            ...
            for i, bar in enumerate(bars):
                width = bar.get_width()
                ax2.text(width, bar.get_y() + bar.get_height() / 2, f" {int(width)}", va="center", fontsize=8)
        ...
        if os_data:
            ...
            for bar in bars:
                width = bar.get_width()
                ax1.text(width, bar.get_y() + bar.get_height() / 2, f" {int(width)}", va="center", fontsize=7)

            # CPU allocation by OS (pie chart)
            colors_pie = plt.cm.Spectral(np.linspace(0, 1, len(os_names)))
            wedges, texts, autotexts = ax2.pie(
                os_cpus, labels=None, autopct="%1.1f%%", colors=colors_pie, startangle=90
            )
        ...
        cluster_data = (
            ...
            .filter(VirtualMachine.cluster.isnot(None))
            .group_by(VirtualMachine.cluster)
            .all()
        )
        ...
        if cluster_data:
            ...
            vms_per_host = [
                (vm_count / host_count) if host_count > 0 else 0 for _, vm_count, host_count, _, _ in cluster_data[:10]
            ]
            ...
            for bar in bars:
                height = bar.get_height()
                ax1.text(
                    bar.get_x() + bar.get_width() / 2, height, f"{height:.1f}", ha="center", va="bottom", fontsize=7
                )
            ...
            scatter = ax2.scatter(
                cluster_cpus,
                cluster_mem,
                s=[v * 5 for v in cluster_vms],
                alpha=0.6,
                c=range(len(cluster_names)),
                cmap="rainbow",
            )
            ax2.set_xlabel("Total vCPUs", fontsize=10)
        ...
        env_data = (
            self.session.query(VirtualMachine.env, func.count(VirtualMachine.id).label("count"))
            .filter(VirtualMachine.env.isnot(None))
            .group_by(VirtualMachine.env)
            .all()
        )
        ...
        if env_data:
            ...
            wedges, texts, autotexts = ax.pie(
                env_counts, labels=env_names, autopct="%1.1f%%", colors=colors_env, startangle=90
            )
    ...
    def _create_extended_comparison_section(self) -> list:
        ...
        dc_data = (
            ...
            .filter(VirtualMachine.datacenter.isnot(None))
            .group_by(VirtualMachine.datacenter)
            .all()
        )
        ...
        if dc_data and len(dc_data) > 1:
            ...
            for bar in bars:
                height = bar.get_height()
                ax1.text(
                    bar.get_x() + bar.get_width() / 2, height, f"{int(height)}", ha="center", va="bottom", fontsize=7
                )
    ...
    def _create_extended_folder_analytics_section(self) -> list:
        ...
        if folder_stats and len(folder_stats) > 1:
            ...
            df_folders = pd.DataFrame(
                folder_stats,
                columns=["Folder", "VMs", "Total_CPUs", "Total_Memory_MB", "Total_Provisioned_MiB", "Total_In_Use_MiB"],
            )
            ...
            bars = ax1.barh(range(len(folder_names_storage)), df_top_storage["Total_Provisioned_GB"], color="#fd7e14")
            ...
            for i, bar in enumerate(bars):
                width = bar.get_width()
                ax1.text(width, bar.get_y() + bar.get_height() / 2, f" {width:.0f} GB", va="center", fontsize=6)
            ...
            width = 0.35
            ax2.bar(
                x - width / 2, df_top_storage["Total_Provisioned_GB"], width, label="Provisioned", color="lightsalmon"
            )
            ax2.bar(x + width / 2, df_top_storage["Total_In_Use_GB"], width, label="In Use", color="lightseagreen")
            ...
            bars = ax4.barh(range(len(folder_names_storage)), df_top_storage["Avg_Provisioned_GB"], color="#6f42c1")
            ...
            for i, bar in enumerate(bars):
                width = bar.get_width()
                ax4.text(width, bar.get_y() + bar.get_height() / 2, f" {width:.1f}", va="center", fontsize=6)
