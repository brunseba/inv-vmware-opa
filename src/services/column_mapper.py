...
class ColumnMapper:
    ...
    def __init__(self, custom_config: ColumnMappingConfig | None = None, use_defaults: bool = True):
    ...
    def map_columns(
        self, df: pd.DataFrame, expected_fields: set[str] | None = None
        ...
        normalized_to_original = {self._normalize_column_name(col): col for col in df.columns}
    ...
    def generate_template(
        excel_file: Path, output_file: Path, sheet: str | int = 0, include_samples: bool = True
    ) -> None:
        ...
        if output_file.suffix in [".yaml", ".yml"]:
            ...
            # Add header comment
            header = """# Column Mapping Configuration for {excel_file.name}
#
# Instructions:
