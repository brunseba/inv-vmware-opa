...
class NamingConventionService:
    ...
    def update_convention(
        ...
        if name is not None:
            ...
            existing = self.session.query(NamingConvention).filter(
                NamingConvention.name == name,
                NamingConvention.id != convention_id
            ).first()
            if existing:
                raise NamingConventionError(f"Convention with name '{name}' already exists")
        ...
        if fields is not None:
            ...
            self._validate_pattern_and_fields(
                pattern or convention.pattern,
                fields
            )
            
            # Delete existing fields
            self.session.query(NamingConventionField).filter_by(
    ...
    def parse_vm_name(
        ...
        if convention.total_length and len(vm_name) != convention.total_length:
            errors.append(
                f"VM name length {len(vm_name)} does not match expected length "
                f"{convention.total_length}"
            )
        ...
        for field in fields:
            if current_pos + field.length > len(vm_name):
                errors.append(
                    f"VM name too short to extract field '{field.field_name}' "
                    f"at position {field.position}"
                )
            ...
            if field.possible_values and isinstance(field.possible_values, list):
                if value not in field.possible_values:
                    errors.append(
                        f"Field '{field.field_name}' value '{value}' not in allowed values: "
                        f"{field.possible_values}"
                    )
    ...
    def query_vms_by_field(
        ...
        """
        query = self.session.query(VMNamingAnalysis).filter_by(
            convention_id=convention_id
        )
    ...
    def generate_migration_groups(
        ...
        analyses = self.session.query(VMNamingAnalysis).filter_by(
            convention_id=convention_id,
            is_valid=True
        ).all()
        ...
        for analysis in analyses:
            ...
            group_key = tuple(
                analysis.field_values.get(field, None)
                for field in grouping_fields
            )
            
            if group_key not in groups:
                groups[group_key] = {
                    'group_key': dict(zip(grouping_fields, group_key)),
                    'vm_count': 0,
                    'vm_ids': []
                }
    ...
    def _validate_pattern_and_fields(self, pattern: str, fields: List[Dict]) -> None:
        ...
        for field in fields:
            if field['length'] <= 0:
                raise PatternValidationError(
                    f"Field '{field['field_name']}' length must be positive"
                )
