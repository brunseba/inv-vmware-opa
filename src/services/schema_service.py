...
class SchemaService:
    ...
    def get_current_version(self) -> SchemaVersion | None:
        ...
        try:
            return (
                self.session.query(SchemaVersion)
                .filter(SchemaVersion.is_current == True)
    ...
    def get_all_versions(self) -> list[SchemaVersion]:
        ...
        """
        return self.session.query(SchemaVersion).order_by(SchemaVersion.applied_at.desc()).all()
