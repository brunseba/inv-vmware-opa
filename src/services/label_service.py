...
class LabelService:
    ...
    def create_label(self, key: str, value: str, description: str = None, color: str = None) -> Label:
        """Create a new label definition."""
        # Check if label already exists
        existing = self.session.query(Label).filter(Label.key == key, Label.value == value).first()

        if existing:
            return existing

        label = Label(key=key, value=value, description=description, color=color)
    ...
    def assign_vm_label(
        self, vm_id: int, label_id: int, assigned_by: str = None, inherited: bool = False, source_folder: str = None
    ) -> VMLabel:
        """Assign a label to a VM."""
        # Check if already assigned
        existing = self.session.query(VMLabel).filter(VMLabel.vm_id == vm_id, VMLabel.label_id == label_id).first()
    ...
    def remove_vm_label(self, vm_id: int, label_id: int) -> bool:
        ...
        vm_label = self.session.query(VMLabel).filter(VMLabel.vm_id == vm_id, VMLabel.label_id == label_id).first()
    ...
    def clean_all_vm_labels(self, include_inherited: bool = True) -> Dict[str, int]:
        ...
        Args:
            include_inherited: If True, remove inherited labels too. If False, only remove direct assignments.

        Returns:
            Dictionary with statistics: total_removed, direct_removed, inherited_removed
        """
        query = self.session.query(VMLabel)

        if not include_inherited:
            query = query.filter(VMLabel.inherited_from_folder.is_(False))

        # Get counts before deletion
        total_count = query.count()
        inherited_count = (
            (self.session.query(VMLabel).filter(VMLabel.inherited_from_folder.is_(True)).count())
            if include_inherited
            else 0
        )
        direct_count = total_count - inherited_count
    ...
    def get_vm_labels(self, vm_id: int, include_inherited: bool = True) -> List[Dict]:
        ...
        query = (
            ...
            .join(VMLabel, Label.id == VMLabel.label_id)
    ...
    def get_vms_with_label(self, key: str, value: str) -> List[VirtualMachine]:
        ...
        return (
            ...
            .join(VMLabel, VirtualMachine.id == VMLabel.vm_id)
            .filter(VMLabel.label_id == label.id)
            .all()
        )
    ...
    def assign_folder_label(
        self,
        folder_path: str,
        label_id: int,
        assigned_by: str = None,
        inherit_to_vms: bool = True,
        inherit_to_subfolders: bool = False,
    ) -> FolderLabel:
        """Assign a label to a folder."""
        # Check if already assigned
        existing = (
            self.session.query(FolderLabel)
            .filter(FolderLabel.folder_path == folder_path, FolderLabel.label_id == label_id)
            .first()
        )
    ...
    def remove_folder_label(self, folder_path: str, label_id: int, remove_inherited: bool = True) -> bool:
        ...
        if remove_inherited:
            self._remove_inherited_labels_from_vms(folder_path, label_id, folder_label.inherit_to_subfolders)
    ...
    def get_folder_labels(self, folder_path: str) -> List[Dict]:
        ...
        results = (
            ...
            .filter(FolderLabel.folder_path == folder_path)
            .all()
        )
    ...
    def get_folders_with_label(self, key: str, value: str) -> List[str]:
        ...
        results = self.session.query(FolderLabel.folder_path).filter(FolderLabel.label_id == label.id).all()
    ...
    def get_all_folders(self) -> List[str]:
        ...
        results = (
            ...
            .distinct()
            .order_by(VirtualMachine.folder)
            .all()
        )
    ...
    def get_folder_stats(self, folder_path: str) -> Dict:
        ...
        vm_count = (
            self.session.query(func.count(VirtualMachine.id)).filter(VirtualMachine.folder == folder_path).scalar() or 0
        )

        # Storage sum
        storage_sum = (
            self.session.query(func.sum(VirtualMachine.in_use_mib))
            .filter(VirtualMachine.folder == folder_path)
            .scalar()
            or 0
        )
    ...
    def _apply_folder_label_to_vms(self, folder_path: str, label_id: int, include_subfolders: bool):
        ...
        if include_subfolders:
            ...
            vms = (
                self.session.query(VirtualMachine)
                .filter(or_(VirtualMachine.folder == folder_path, VirtualMachine.folder.like(f"{folder_path}/%")))
                .all()
            )
        else:
            # Exact match only
            vms = self.session.query(VirtualMachine).filter(VirtualMachine.folder == folder_path).all()

        # Assign label to each VM (if not already assigned)
        for vm in vms:
            try:
                self.assign_vm_label(
                    vm_id=vm.id, label_id=label_id, assigned_by="system", inherited=True, source_folder=folder_path
                )
            except Exception:
    ...
    def _remove_inherited_labels_from_vms(self, folder_path: str, label_id: int, include_subfolders: bool):
        """Remove inherited labels from VMs in a folder."""
        query = (
            self.session.query(VMLabel)
            .join(VirtualMachine, VMLabel.vm_id == VirtualMachine.id)
            .filter(
                VMLabel.label_id == label_id,
                VMLabel.inherited_from_folder.is_(True),
        ...
        if include_subfolders:
            query = query.filter(
                or_(VirtualMachine.folder == folder_path, VirtualMachine.folder.like(f"{folder_path}/%"))
            )
        else:
    ...
    def sync_inherited_labels(self, folder_path: str = None):
        ...
        if folder_path:
            folder_labels = (
                self.session.query(FolderLabel)
                .filter(FolderLabel.folder_path == folder_path, FolderLabel.inherit_to_vms.is_(True))
                .all()
            )
        else:
            folder_labels = self.session.query(FolderLabel).filter(FolderLabel.inherit_to_vms.is_(True)).all()

        # Reapply each folder label
        for fl in folder_labels:
            self._apply_folder_label_to_vms(fl.folder_path, fl.label_id, fl.inherit_to_subfolders)
    ...
    def get_vm_effective_labels(self, vm_id: int) -> Dict[str, Dict]:
        ...
        """
        # Get VM
        vm = self.session.query(VirtualMachine).filter(VirtualMachine.id == vm_id).first()
    ...
    def get_vms_by_os_category(self, os_pattern: str = None, os_family: str = None) -> List[VirtualMachine]:
        ...
        if os_pattern:
            ...
            if os_family.lower() == "windows":
                query = query.filter(
                    or_(VirtualMachine.os_config.like("%Windows%"), VirtualMachine.os_config.like("%Microsoft%"))
                )
    ...
    def get_vms_by_resource_criteria(
        self,
        min_cpus: int = None,
        max_cpus: int = None,
    ...
    def get_distinct_os_values(self) -> List[str]:
        ...
        """
        result = (
            self.session.query(VirtualMachine.os_config)
            .filter(VirtualMachine.os_config.isnot(None))
            .distinct()
            .order_by(VirtualMachine.os_config)
            .all()
        )
    ...
    def get_vm_counts_by_criteria(self, os_pattern: str = None, resource_category: str = None) -> Dict:
        ...
        """
        counts = {"os_family": {}, "resource_size": {}, "network_complexity": {}, "storage_complexity": {}}
