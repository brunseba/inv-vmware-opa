...
class MigrationScenarioService:
    ...
    def create_target(self, name: str, platform_type: PlatformType, region: str, **kwargs) -> MigrationTarget:
        """Create a new migration target."""
        target = MigrationTarget(name=name, platform_type=platform_type, region=region, **kwargs)
    ...
    def get_active_targets(self) -> list[MigrationTarget]:
        ...
        return self.session.query(MigrationTarget).filter(MigrationTarget.is_active == True).all()
    ...
    def delete_scenarios_bulk(self, scenario_ids: list[int]) -> int:
        ...
        """
        deleted_count = (
            self.session.query(MigrationScenario)
            .filter(MigrationScenario.id.in_(scenario_ids))
    ...
    def calculate_migration_cost(
        ...
        strategy_config = (
            self.session.query(MigrationStrategyConfig).filter(MigrationStrategyConfig.strategy == strategy).first()
        )
        ...
        migration_labor = len(vms) * strategy_config.hours_per_vm * strategy_config.labor_rate_per_hour
        ...
        if strategy_config.memory_multiplier > 0:
            runtime_memory_monthly = (
                total_memory_gb * target.memory_cost_per_gb * 24 * 30 * strategy_config.memory_multiplier
            )

        runtime_storage_monthly = 0.0
        if strategy_config.storage_multiplier > 0:
            runtime_storage_monthly = total_storage_gb * target.storage_cost_per_gb * strategy_config.storage_multiplier
        ...
        runtime_total_monthly = (
            runtime_compute_monthly + runtime_memory_monthly + runtime_storage_monthly + runtime_saas_monthly
        )
    ...
    def calculate_migration_duration(
        ...
        effective_storage_gb = total_storage_gb * compression_ratio * dedup_ratio * network_overhead
        ...
        total_replication_hours = (initial_replication_hours + delta_sync_hours) * replication_efficiency
    ...
    def assess_risk_level(
        self, vms: list[VirtualMachine], target: MigrationTarget, strategy: MigrationStrategy
    ) -> tuple[str, list[str]]:
    ...
    def calculate_recommendation_score(
        self, cost_breakdown: dict[str, float], duration: dict[str, float], risk_level: str, target: MigrationTarget
    ) -> float:
        ...
        # Risk factor
        risk_penalties = {"CRITICAL": 30, "HIGH": 20, "MEDIUM": 10, "LOW": 0}
    ...
    def create_scenario(
        ...
        **kwargs,
        ...
        # Calculate estimates
        cost_breakdown = self.calculate_migration_cost(vms, target, kwargs.get("duration_days", 30), strategy)
        duration = self.calculate_migration_duration(vms, target, kwargs.get("parallel_migrations", 5), strategy)
        risk_level, risk_factors = self.assess_risk_level(vms, target, strategy)

        # Calculate recommendation score
        recommendation_score = self.calculate_recommendation_score(cost_breakdown, duration, risk_level, target)
    ...
    def compare_scenarios(self, scenario_ids: list[int]) -> pd.DataFrame:
        """Compare multiple scenarios side-by-side."""
        scenarios = self.session.query(MigrationScenario).filter(MigrationScenario.id.in_(scenario_ids)).all()
    ...
    def _get_vms_by_criteria(self, criteria: dict) -> list[VirtualMachine]:
        ...
        if "labels" in criteria:
            ...
            for label_spec in criteria["labels"]:
                ...
                label_subquery = (
                    select(VMLabel.vm_id)
                    .join(Label, VMLabel.label_id == Label.id)
                    .filter(Label.key == label_spec["key"], Label.value == label_spec["value"])
                    .scalar_subquery()
                )
    ...
    def generate_migration_waves(
        self, scenario_id: int, wave_size: int = 10, strategy: str = "size_based"
    ) -> list[MigrationWave]:
