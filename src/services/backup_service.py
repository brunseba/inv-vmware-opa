...
class BackupService:
    ...
    def export_labels(self, output_path: Path, include_metadata: bool = True) -> dict:
        ...
        vm_labels = (
            self.session.query(VMLabel, VirtualMachine.vm)
        ...
        for folder_label in folder_labels:
            ...
            if include_metadata:
                folder_assignment["assigned_at"] = (
                    folder_label.assigned_at.isoformat() if folder_label.assigned_at else None
                )
    ...
    def import_labels(self, input_path: Path, mode: str = "merge", clear_existing: bool = False) -> dict:
        """
        ...
        try:
            ...
            for label_data in import_data.get("labels", []):
                ...
                existing = (
                    ...
                    .filter(Label.key == label_data["key"], Label.value == label_data["value"])
                    .first()
                )
            ...
            for vm_assignment in import_data.get("vm_assignments", []):
                try:
                    ...
                    # Get VM
                    vm = self.session.query(VirtualMachine).filter(VirtualMachine.vm == vm_name).first()
                    ...
                    existing = (
                        ...
                        .filter(VMLabel.vm_id == vm.id, VMLabel.label_id == new_label_id)
                        .first()
                    )
            ...
            for folder_assignment in import_data.get("folder_assignments", []):
                try:
                    ...
                    existing = (
                        ...
                        .filter(FolderLabel.folder_path == folder_path, FolderLabel.label_id == new_label_id)
                        .first()
                    )
    ...
    def restore_database(self, backup_path: Path, db_url: str, confirm: bool = False) -> dict:
        """
